...
def choose_projected_crs(sample_metadata):
    ...
    zone = int((center_lon + 180.0) // 6.0) + 1 #calculates UTM zone number.
    epsg = 32600 + zone if center_lat >= 0 else 32700 + zone #chooses either northern or southern hemisphere.

    return CRS.from_epsg(epsg)


def VISTA_DBSCAN(sample_metadata, max_distance_m):
    """
    ...
    transformer = Transformer.from_crs(
        ...
        projected_crs,
        always_xy=True,
    ) #converts coordinate system to use metres.
    ...
    for row in sample_metadata:
        ...
        coords.append((x, y)) #store each coordinate as an x and y metre position.
    ...
    clustering = DBSCAN(
        ...
        min_samples=1,
    ).fit(coords) #run DBSCAN, points within 2 visibility radii are treated as the same group.

    labels = clustering.labels_ #retrieve cluster label for each point.
    ...
    for label in sorted(set(labels)):
        ...
        chunks.append(chunk) #build chunks by collating points with the same cluster label.
